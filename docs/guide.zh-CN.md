...
implementation("com.github.oppsgo.json-kit:json-kotlin:1.0.4") // 可选 Kotlin Instantiator
```
...
| **Fastjson 1.x** | `json-fastjson` / `FastjsonAdapterFactory` | 仅兼容旧依赖（**不**提供 Kotlin Instantiator） |
...
可选模块 `:support:json-kotlin` 通过缓存的 Java `Constructor` 绑定惯用 Kotlin `data class` / 主构造类型（Fastjson2 风格 STATE）。

### 启用

```kotlin
JsonKitKotlin.enable() // 进程启动时调用一次
JsonKit.setDefault(Fastjson2AdapterFactory.of())
```

也可在注册 Instantiator 后，用 `JsonOptions.Builder().setKotlinSupport(true)` 按 Adapter 开启。

### 依赖（由业务方自行声明）

| STATE | 含义 | 业务依赖 |
|-------|------|----------|
| 0 | Java / Instantiator 未启用 | — |
| 1 | `kotlin.Metadata` + stdlib | `json-kotlin` + `kotlin-stdlib`（≥ **1.6.21**） |
| 2 | STATE 1 + reflect 丰富参数名 | 再加 `kotlin-reflect`（与业务 Kotlin 版本一致） |

`json-kotlin` 对 `kotlin-stdlib` / `kotlin-reflect` 为 **compileOnly**，不会强制 API 传递。`:core` 无 Kotlin 运行时硬依赖。

### 注解（v1）

使用 `@field:` 让注解落在 Java field 上（`BindingMeta` 可读）：
...
| 引擎 | Kotlin Instantiator |
|------|---------------------|
| Fastjson2 / Moshi / Gson | `JsonKitKotlin.enable()` 后可用 |
| Fastjson 1.x | **不在范围内** —— 请改用 Fastjson2 |

### R8 / ProGuard（Kotlin）

`json-kotlin` 内含 `META-INF/proguard/jsonkit-kotlin.pro`（保留 `kotlin.Metadata` 与构造）。业务 Kotlin 模型也需 keep：
...
3. Java 模型用 **public 字段 + 无参构造**；Kotlin `data class` 请加 `json-kotlin` 并 `JsonKitKotlin.enable()`。
...
6. 新项目 Fastjson 线用 **Fastjson2**；1.x 仅兼容（**不支持** Kotlin Instantiator）。
7. ProGuard / R8：`:core` 的 JAR 已内嵌极简规则（保留注解属性与 JsonKit 注解类型）。使用 Kotlin 时另有 `jsonkit-kotlin.pro`。**业务 Model、策略类**仍需在 App 中自行 keep（与单独用 Gson 相同）。
...
使用 `:support:json-kotlin` 时另保留 `kotlin.Metadata`（见 `jsonkit-kotlin.pro`）及 Kotlin 模型构造。
